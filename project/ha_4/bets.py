...
class StraightBet(Bet):
    ...
    def validate(self):
        return self.number in list(range(0, 36)).append("00")
...
class ColumnBet(Bet):
    ...
    def validate(self):
        is_on_board = self.line_number < 4 and self.line_number > 0
        return self.column_number % 12 == 1 and is_on_board

    def is_winning(self, winning_number):
        numbers_in_line = list(range(self.column_number, self.column_number + 12))
        return winning_number in numbers_in_line
...
class BetFactory:
    ...
    def create_bet(bet_type, amount, **kwargs):
        if bet_type == "straight":
            return StraightBet(amount, kwargs["number"])
        elif bet_type == "split":
            return SplitBet(amount, kwargs["numbers"])
        elif bet_type == "street":
            return StreetBet(amount, kwargs["street_number"])
        elif bet_type == "corner":
            return CornerBet(amount, kwargs["numbers"])
        elif bet_type == "line":
            return LineBet(amount, kwargs["line_number"])
        elif bet_type == "column":
            return ColumnBet(amount, kwargs["column_number"])
        elif bet_type == "color":
            return ColorBet(amount, kwargs["color"])
        elif bet_type == "even_odd":
            return EvenOddBet(amount, kwargs["even_odd"])
        elif bet_type == "high_low":
            return HighLowBet(amount, kwargs["high_low"])
        else:
