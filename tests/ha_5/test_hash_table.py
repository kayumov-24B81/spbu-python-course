...
class TestHashTable:
    ...
    def test_collision_distribution_basic(self, no_resize_table, table_size, keys):
        """Test that all keys are stored correctly despite collisions."""
        no_resize_table._table_size = table_size
        no_resize_table._buckets = [[] for _ in range(table_size)]
    ...
    def test_collision_distribution_advanced(
        self, no_resize_table, table_size, keys, expected_conditions
    ):
    ...
    def test_auto_resize_scenarios(self, initial_size, load_factor, elements_to_add):
        """Test automatic resizing in different scenarios."""
        table = HashTable(initial_size=initial_size, load_factor=load_factor)
        original_size = table._table_size

        for i in range(elements_to_add):
            table[f"key_{i}"] = i

        assert table._table_size == original_size

        table["resize_trigger"] = "resize!!!"

        assert table._table_size == 2 * original_size
    ...
    def test_no_resize_scenarios(self, initial_size, load_factor, elements_to_add):
...
def test_mixed_operations_sequences(operations, expected_final_state):
